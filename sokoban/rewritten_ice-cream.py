...
def render_map(game):
	map_matrix, x, y = game.map, game.pos.x, game.pos.y
	if game.on_target:
		map_matrix[y][x] = '+'
	elif game.on_door:
		map_matrix[y][x] = 'd'
...
class Game:
	def __init__(self, level):
		self.map = list(map(list, level.split('\n')))
		self.width = len(level.split('\n')[0])
		self.height = len(self.map)
		player = level.find('@')
		self.pos = Pos(player % (self.width+1), player // (self.width+1))
		self.map[self.pos.y][self.pos.x] = ' '
		self.door = False
...
def up(game):
	...
	if game.map[game.pos.y - 1][game.pos.x] == '#':
		return
	elif game.map[game.pos.y - 1][game.pos.x] in 'O*o':
		if game.pos.y - 2 < 1:
			return
		if game.map[game.pos.y - 2][game.pos.x] in '#O*o':
			return
		if game.map[game.pos.y - 2][game.pos.x] == '.':
			game.map[game.pos.y - 2][game.pos.x] = '*'
		elif game.map[game.pos.y - 2][game.pos.x] == 'D':
			if not game.has_key:
				return
		...
		if game.map[game.pos.y - 1][game.pos.x] == 'o':
			game.on_door = True
		else:
			game.on_door = False
	elif game.map[game.pos.y - 1][game.pos.x] == 'D':
		if not game.has_key:
			return
		game.on_door = True
	elif game.map[game.pos.y - 1][game.pos.x] == '.':
		game.on_door = False
		game.on_target = True
	else:
		game.on_door = False
...
def left(game):
	...
	if game.map[game.pos.y][game.pos.x - 1] == '#':
		return
	elif game.map[game.pos.y][game.pos.x - 1] in 'O*o':
		if game.pos.x - 2 < 1:
			return
		if game.map[game.pos.y][game.pos.x - 2] in '#O*o':
			return
		if game.map[game.pos.y][game.pos.x - 2] == '.':
			game.map[game.pos.y][game.pos.x - 2] = '*'
		elif game.map[game.pos.y][game.pos.x - 2] == 'D':
			if not game.has_key:
				return
		...
		if game.map[game.pos.y][game.pos.x - 1] == '*':
			...
			game.on_target = False
		if game.map[game.pos.y][game.pos.x - 1] == 'o':
			game.on_door = True
		else:
			game.on_door = False
	elif game.map[game.pos.y][game.pos.x - 1] == 'D':
		if not game.has_key:
			return
		game.on_door = True
	elif game.map[game.pos.y][game.pos.x - 1] == '.':
		game.on_door = False
		game.on_target = True
	else:
		game.on_door = False
...
def down(game):
	if game.pos.y > game.height - 2:
		return
	if game.map[game.pos.y + 1][game.pos.x] == '#':
		return
	elif game.map[game.pos.y + 1][game.pos.x] in 'O*o':
		game.on_door = False
		if game.pos.y + 2 > game.height - 1:
			return
		if game.map[game.pos.y + 2][game.pos.x] in '#O*o':
			return
		if game.map[game.pos.y + 2][game.pos.x] == '.':
...
def right(game):
	if game.pos.x > game.width - 2:
		return
	if game.map[game.pos.y][game.pos.x + 1] == '#':
		return
	elif game.map[game.pos.y][game.pos.x + 1] in 'O*o':
		if game.pos.x + 2 > game.width - 1:
			return
		if game.map[game.pos.y][game.pos.x + 2] in '#O*o':
			return
		if game.map[game.pos.y][game.pos.x + 2] == '.':
...
def create_door(game, x, y):
	if game.door:
		...
		return
	game.door = True
	if game.pos.x + x < 1 or game.pos.x + x > game.width - 2:
		return
	if game.pos.y + y < 1 or game.pos.y + y > game.height - 2:
		return
	if game.map[game.pos.y + y][game.pos.x + x] != '#':
		return
	game.map[game.pos.y + y][game.pos.x + x] = 'D'
	possible = []
